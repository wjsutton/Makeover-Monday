# Makeover Monday Week 32 2020
# Benefits of Remote Work
# Data Source: State of Remote Work 2020

#library("httr")
#library("readxl")
#GET("https://query.data.world/s/524igbtttdrn3gfibs2eqfmd7iiqwg", write_disk(tf <- tempfile(fileext = ".xlsx")))
#            df <- read_excel(tf)

# Load libraries
library(ggplot2)
library(reshape2)
library("httr")
library("readxl")

# GET data
GET("https://query.data.world/s/524igbtttdrn3gfibs2eqfmd7iiqwg", write_disk(tf <- tempfile(fileext = ".xlsx")))
df <- read_excel(tf)

data <- data.frame(
  name=df$`Benefit of remote work`,  
  value=df$Percentage)

barplot <- ggplot(data, aes(x=reorder(name, value), y=value)) + # enter data frame
  geom_bar(stat = "identity"
           ,fill = "#7f7fff") + # colour bars
  coord_flip() + # flip vertical bars into horizontal bar
  geom_text(aes(label = paste0(value,'%') # data labels
                ,y =  value + max(value)*0.01) # positioned just outside bar
            ,hjust = 0 # left align text
            ,size = (5/14)*8 # sizing in line with theme text
            ,colour = "#323232") + # colour text
  theme(
    panel.grid.major = element_blank() # Remove gridlines (major)
    ,panel.grid.minor = element_blank() # Remove gridlines (minor)
    ,panel.background = element_blank() # Remove grey background
    ,plot.title = element_text(hjust = 0, size = 16, colour = "#323232") # Title size and colour
    ,plot.title.position = "plot"
    ,plot.subtitle = element_text(hjust = 0, size = 8, colour = "#323232") # Subtitle size and colour
    ,plot.caption = element_text(vjust = 0.3, size = 8, colour = "#323232") # Caption size and colour
    ,axis.ticks.y = element_blank() # Remove tick marks (Y-Axis)
    ,axis.text.y = element_text(hjust = 1, colour = "#323232", size = 8) # Axis size and colour (Y-Axis)
    ,axis.ticks.x = element_blank() # Remove tick marks (X-Axis)
    ,axis.text.x  = element_blank() # Remove axis scale (X-Axis)
    ,axis.title.x = element_text(size = 8, colour = "#323232"
                                 ) # Axis label size and colour (X-Axis)
  ) +
  geom_hline(yintercept=0, color = "#7f7fff", size=1) + # Axis line size and colour (Y-Axis)
  labs(title = 'The Flex Life' # Title text
       ,subtitle = lapply(strwrap("Data from Buffer shows flexibility (in schedule & location) as the top benefit to remote working.", simplify = FALSE), paste, collapse="\n") # Subtitle text
       ,x = element_blank() # Removed Y-Axis text (flipped co-ordinates) 
       ,y = element_blank() # 'Response (%)' # X-Axis text (flipped co-ordinates) 
       ,caption = "Author: Will Sutton, Source: Buffer - State of Remote Work 2020") + # Caption text
  scale_y_continuous(limits = c(
    ifelse(min(data$value)>=0,0,min(data$value)*1.05) # Minimum X-Axis scale (flipped co-ordinates)
    ,max(data$value)*1.25) # Maximum X-Axis scale (flipped co-ordinates)
    ,expand = c(0,0) # Removing blank space around plot (X-Axis (flipped co-ordinates))
  )

# Save the plot
ggsave("the_flex_life_bar_chart.png" # filename
       ,plot = barplot # variable for file
       ,width = 5, height = 4, dpi = 300, units = "in") # dimensions and image quality